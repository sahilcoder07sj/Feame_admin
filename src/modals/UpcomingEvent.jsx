...
import { upcomingEvent as eventSchema } from '../lib/schema';
...
const UpcomingEvent = () => {
  ...
  const addEventCategoryOptions = [
    { value: 'upcoming', label: 'Upcoming' }
  ];
  const updateEventCategoryOptions = [
    { value: 'upcoming', label: 'Upcoming' },
  ...
  const event_type_watch = eventForm.watch('type');
  ...
  useEffect(() => {
    ...
    if (event_type_watch == "live") {
      // if (data.end_time && data.end_time != null) {
      //   eventForm.reset({ ...eventSchema.parse(fillterData), start_time: convertUTCToLocalTime(data.start_time), end_time: convertUTCToLocalTime(data.end_time) });
      // } else {
      //   eventForm.reset({ ...eventSchema.parse(fillterData), start_time: convertUTCToLocalTime(data.start_time) });
      // }
      if (data.end_time && data.end_time != null) {
      ...
      eventForm.reset(eventSchema.parse(data))
  ...
  const onSubmit = values => {
    ...
    if (event_type_watch == "live") {
      ...
      const { start_time, end_time, email, ...updateValues } = values;
      const filterImages = eventImages.filter((item) => item.upload_type == "new");

      const updatedEmail = convertToCommaSeparatedString(email)
      // if (values.end_time) {
      //   mutate({ ...updateValues, event_timezone: eventTimeZone, ...updatedWhereTag,...updatedTags, ...updatedKeywords, email: updatedEmail, ...(search.mode === DIALOG_MODE.add ? {} : { event_id: search.event_id }), allow_user_ids: convertArrayToString(selectedUserIds), admin_id, start_time: convertLocalTimeToUTC(values.start_time), end_time: convertLocalTimeToUTC(values.end_time), ...(filterImages.length > 0 ? { event_images: filterImages } : {}), delete_event_images: search.mode == DIALOG_MODE.add ? "" : removeUploadIds.join(',') });
      // } else {
      //   mutate({ ...updateValues, event_timezone: eventTimeZone, ...updatedWhereTag,...updatedTags, ...updatedKeywords, email: updatedEmail, ...(search.mode === DIALOG_MODE.add ? {} : { event_id: search.event_id }), allow_user_ids: convertArrayToString(selectedUserIds), admin_id, start_time: convertLocalTimeToUTC(values.start_time), ...(filterImages.length > 0 ? { event_images: filterImages } : {}), delete_event_images: search.mode == DIALOG_MODE.add ? "" : removeUploadIds.join(',') });
      // }
      if (values.end_time) {
      ...
      const filterValue = _.pick(values, ["name", "event_profile", "click_img", "subtitle", "type", "city", 'lat', 'lng', 'location', 'event_type_id'])
      mutate({
        ...filterValue,
        allow_user_ids: convertArrayToString(selectedUserIds),
        ...(search.mode === DIALOG_MODE.add ? {} : { event_id: search.event_id }),
        admin_id
      });
    }
            ...
            <form noValidate onSubmit={eventForm.handleSubmit(onSubmit, onError)} className='h-full pt-0 flex flex-col gap-y-4 overflow-hidden'>
              <ScrollArea className='h-full px-5'>
                <div className='p-1 space-y-4'>
                  ...
                  <div>
                    <SelectWithLabel label='Event Category' name='type' placeholder='Event Category' option={search.mode === DIALOG_MODE.add ? addEventCategoryOptions : updateEventCategoryOptions} />
                  </div>
                  ...
                  <InputWithLabel label='Subtitle' name='subtitle' placeholder='Enter Subtitle' />
                  {/* <InputWithLabel label='City' name='upcoming_event_city' placeholder='Enter city' /> */}

                  {event_type_watch == "live" && <>
                    <div className='w-full flex flex-col gap-y-2'>
                      <FormLabel htmlFor='eventprofile' className='text-lg dark:text-text_main'>
                        Event images
                      </FormLabel>
