...
import { ADDDISCOUNTLIST, GETDISCOUNTLIST, GETEVENTTYPELIST, GETSINGLEDISCOUNTLIST, GETUSERFORNOTIFY, UPDATEDISCOUNTLIST } from '@/lib/endpoint';
...
const Discount = () => {
  ...
  const { data, isFetching } = useQuery({
    queryKey: [GETSINGLEDISCOUNTLIST, search.event_id],
    queryFn: () => privateAxios({ url: GETSINGLEDISCOUNTLIST, data: { event_id: search.event_id } }),
    enabled: typeof search.event_id === 'number',
    select: res => {

      // res.data.name = res.data.where_dtl.map(tag => ({ id: tag.event_where_id, text: tag.where_tag }));
      ...
      // return parsedData;
      // Mapping the response to parsed data structure for form
      const parsedData = {
        id: res.data.id,
        name: res.data.name,
        ...
        event_lat: res.data.event_lat,
        event_lng: res.data.event_lng,
        discount_code: res.data.discount_code,
        discount_amount: res.data.discount_amount,
        subtitle: res.data.subtitle,
        discount_images: res.data.discount_images,
  ...
  useEffect(() => {
    ...
    // Extract the event profile and discount images
    // const discountImages = data.discount_images && data.discount_images.length > 0
    //   ? data.discount_images[0].media
    //   : ''; // If discount_images is empty, set it to an empty array
    // Extract the last discount image from the array if it exists, otherwise set it to an empty string
    const discountImages = data.discount_images && data.discount_images.length > 0
      ? data.discount_images[data.discount_images.length - 1].media // Select the last image if available
      : ''; // If no discount images, set it to an empty string


    // Reset form fields with parsed response data
    eventForm.reset({
      name: data.name,
      ...
      event_profile: data.event_profile, // Directly set the event profile URL
      click_img: discountImages, // Use constructed discountImages array
      // discount_images: data.discount_images.map(img => ({
      //   ...img,
      //   upload_type: 'old'
      // })),
      // Handle old images and construct the full path
  ...
  }, [isOpen, data, isFetching, search.event_id]);
  ...
  const onClose = () => {
    ...
    delete closeModal.event_id;  // Reset `event_id` after update
    delete closeModal.discount_id; // Reset `discount_id` after update
    setEventImages([]);
    setRemoveUploadIds([]);
    eventForm.reset(defaultValues);
    navigate({ search: closeModal });
    setRemovedTagIds([]);
    // setRemovedKeywordsIds([]);
    setRemovedTagsIds([]);
  };
  ...
  const onSubmit = values => {
    ...
    console.log(values);
    const isEditMode = search.mode === DIALOG_MODE.edit && search.event_id;

    // Build the data object for add/update, adding IDs only in edit mode and include `type: 'live'`
    const data = {
      ...values,
      admin_id,
      type: 'live', // Always include `type: 'live'`
      ...(isEditMode ? { discount_id: search.discount_id, event_id: search.event_id } : {})
    };

    // Handle `event_profile` (Photo) field
    if (data.event_profile) {
      // If a new image is selected (a file), pass the file object
      data.event_profile = data.event_profile instanceof File
        ? data.event_profile  // If it's a file (binary), keep the file object
        : '';  // Don't pass the existing URL if no new image is selected
    } else if (isEditMode) {
      // If event_profile is empty in edit mode, leave it blank (don't pass the old URL)
      data.event_profile = '';
    }
    ...
    // Rename click_img to event_images[]
    // if (data.click_img) {
    //   data['event_images[]'] = Array.isArray(data.click_img) ? data.click_img : [data.click_img];
    //   delete data.click_img;
    // }
    // Handle `click_img` field
    if (data.click_img) {
      // If a new image is selected (a file), pass the file object
      data['event_images[]'] = Array.isArray(data.click_img)
        ? data.click_img
        : [data.click_img instanceof File ? data.click_img : undefined];  // If it's a file, pass the file object
      delete data.click_img; // Rename `click_img` to `event_images[]`
    } else if (isEditMode) {
      // If no new image is selected and we're in edit mode, leave the field blank
      data['event_images[]'] = [];  // Empty array for event_images (do not pass old URL)
    }
    console.log(data);

    // Call mutate with the data object
    mutate(data);
              ...
              <ScrollArea className='h-full px-5'>
                <div className='p-1 space-y-4'>
                  <div className='w-full flex flex-col gap-y-2'>
                    ...
                    <InputWithLabel label='Discount Code' name='discount_code' placeholder='Enter Discount Code' required />
                    <InputWithLabel type="number" label='Discount %' name='discount_amount' placeholder='Enter Discount %' required />
                  ...
                  <div className='space-y-2'>
                    ...
                    {/* <p className='ml-3 md:ml-4 dark:text-red-500 text-[12px] font-semibold '>{eventForm.formState.errors.location?.message}</p> */}
                  ...
                  <div className='space-y-2'>
                    ...
                    />
                    <p className='ml-3 md:ml-4 dark:text-red-500 text-[12px] font-semibold '>{eventForm.formState.errors.event_address?.message}</p>

                  </div>


                  <InputWithLabel label='Subtitle' name='subtitle' placeholder='Next Week!' />
