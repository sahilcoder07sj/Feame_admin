...
import { DELETEUSER, REMOVE_USER_FROM_EVENT } from '@/lib/endpoint';
import useAxios from '@/axios/useAxios';
import { toast } from 'react-toastify';
import Loader from '@/components/common/Loader';

const RemoveUserFromEvent = ({ data, setData, refetch }) => {
      ...
      <Dialog open={data.open} onOpenChange={onClose}>
        <DialogContent onPointerDownOutside={(e) => e.preventDefault()} onOpenAutoFocus={(e) => e.preventDefault()} className='max-w-xl p-6 gap-0 overflow-hidden rounded-2xl'>
          ...
          <DialogHeader className='my-6 gap-y-1 text-center'>
            <DialogTitle className='text-center text-2xl font-semibold dark:text-text_main'>Remove User</DialogTitle>
            <p className='text-center text-lg dark:text-input_text'>Are you sure you want to remove this user from this event?</p>
          </DialogHeader>
