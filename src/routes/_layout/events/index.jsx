...
import { DELETEEVENTLIST, GETEVENTLIST, GETLOCATIONLIST } from '@/lib/endpoint';
import { dataFilterSchema } from '@/lib/schema';
import { cn } from '@/lib/utils';
import { useMutation, useQuery } from '@tanstack/react-query';
import { Link, createFileRoute } from '@tanstack/react-router';
import { BookCopy, Clock, Info, RotateCcw, Search, SlidersHorizontal } from 'lucide-react';
...
import { Route as UpcomingEvent } from './upcoming';
import { Route as EventDetails } from './$event_id';
import Notification from '@/modals/Notification';
import EventUserModal from '@/modals/EventUser';
import { UPCOMING_ICON } from '@/lib/images';
...
function Events() {
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: [GETEVENTLIST, page, limit, EVENTCATEGORY.LIVE, debouncedSearchQuery, dateFilter, locationFilter],
    queryFn: () => privateAxios({
      url: GETEVENTLIST, data: {
        page, limit, search: debouncedSearchQuery, type: EVENTCATEGORY.LIVE,
        ...(dateFilter && {
          order_by_event_date: dateFilter
        }),
        ...(locationFilter && {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: [DELETEEVENTLIST],
    mutationFn: data => privateAxios({ url: DELETEEVENTLIST, data }),
    onSettled: res => {
  ...
  const columns = [
    ...
    {
      name: 'location',
      selector: row => <p className='whitespace-normal'>{row.location}</p>
    },
    {
      name: 'Seat',
      selector: row => <div className='flex gap-x-3 items-center'>
        <p className='whitespace-normal w-11'>{`${row.total_booked_seat} / ${row.total_seat}`} </p>
        <Info onClick={() => setEventUserModal({
          open: true,
          data: row.event_id
        })} className='stroke-[#888888] cursor-pointer w-5' />
      </div>
    },
    {
      name: 'Action',
      selector: row => (
        <div className='space-x-1  flex justify-center'>
          ...
          <Button onClick={() => setCopyEvent({
            open: true,
            data: row
          })} className="!bg-transparent">
            <BookCopy size={20} color='#888888' />
          </Button>
          <Link
            to={EventDetails.to}
  ...
  return (
    <>
      <div className='py-4 px-6 flex justify-between items-center gap-x-4'>
        ...
        <div className='flex gap-4 items-center'>
          <Popover>
            ...
            <PopoverContent align="start" side="left" className="dark:bg-[#26282c] px-2 py-2">
              <div className='flex flex-col gap-4'>
                <div className='flex-1'>
                  <Filter placeholder="Select Order" value={dateFilter} setValue={setDateFilter} option={option} />
                </div>
                <div>
                  <Filter placeholder="Select Location" value={locationFilter} setValue={setLocationFilter} option={eventLocation} />
                </div>
                <Button className='flex gap-2 items-center mx-2' onClick={() => { setDateFilter(''); setLocationFilter('') }} type='button'><span className='text-lg'>Reset</span><RotateCcw size={18} color='black' /></Button>

              </div>
            </PopoverContent>
          </Popover>
          <Link to={UpcomingEvent.to} className={cn(buttonVariants(), 'py-3 rounded-[10px] gap-x-2 font-medium text-lg')}>
            <img className='w-[22px]' src={UPCOMING_ICON} alt="" />
            Upcoming Event
          </Link>
          <Button
      ...
      <DuplicateEvent data={copyEvent} setData={setCopyEvent} refetch={refetch} />
      <Delete
