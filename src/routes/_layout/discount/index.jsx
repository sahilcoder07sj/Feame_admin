...
import { DELETEDISCOUNTLIST, GETDISCOUNTLIST, GETLOCATIONLIST } from '@/lib/endpoint';
...
import { Route as UpcomingDiscount } from './upcoming';
...
export const Route = createFileRoute('/_layout/discount/')({
  component: Discount,
...
function Discount() {
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: [GETDISCOUNTLIST, page, limit, EVENTCATEGORY.LIVE, debouncedSearchQuery, dateFilter, locationFilter],
    queryFn: () => privateAxios({
      url: GETDISCOUNTLIST, data: {
  ...
  const columns = [
    ...
    },
    {
      name: 'Event Address',
      width: '15%',
      selector: row => <p className='whitespace-normal'>{row.event_address}</p>
    },
    {
      name: 'Subtitle',
      width: '16%',
    ...
    },
    {
      ...
      selector: row => (
        ...
        <Button
            ...
            onClick={() => navigate({ search: { page, limit, search, mode: DIALOG_MODE.edit, event_id: Number(row.id) } })}>
    ...
    <>
      <div className='py-4 px-6 flex justify-between items-center gap-x-4'>
        ...
        <div className='flex gap-4 items-center'>
          ...
          {/* <Link to={UpcomingDiscount.to} className={cn(buttonVariants(), 'py-3 rounded-[10px] gap-x-2 font-medium text-lg')}>
      ...
      <Delete
        ...
        onConfirm={() => mutate({ event_id: eventModal.data.id })}
      />
    </>
  );
}
